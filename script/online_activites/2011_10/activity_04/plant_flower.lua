...
Include("\\script\\online_activites\\2011_10\\activity_04\\string.lua") --»î¶¯×Ö·û´®
Include("\\script\\lib\\globalfunctions.lua") --¹«¹²º¯ÊýÎÄ¼þ
Include("\\script\\misc\\taskmanager.lua") 

VET_201111_TIME_USER_DEFINE = 2556 	--Ê±¼ä´¥·¢Æ÷×Ô¶¨ÒåID
VET_201111_TIME_TB_DEFINE = 1514 	--Ê±¼ä´¥·¢Æ÷±íÄÚID
VET_201111_ROSE_SEED_COST = 150000  	--»¨Ãç¼Û¸ñ30½ð
VET_201111_MAX_TIMES = 4  				--Ã¿ÈË×î¶à²ÎÓë»î¶¯4´Î
VET_201111_ROSE_LIFE = 10*60          	--»¨¶änpc´æ»îÊ±¼ä£¬Êµ¼ÊÊ±ÏÞºÍ´¥·¢Æ÷Ê±ÏÞ¶¼ÂÔÎ¢ÓÐ¼Ó³¤ÐÞ¸´
VET_201111_ROSE_ACTIVITY_ID = 45	--»î¶¯ID
--ÈÎÎñ±äÁ¿´¦Àí
...
function to_set_fire_now(nTeamSize)
	...
	if  nResult < 75 then
		...
		SetNpcLifeTime(nNpcIndex, VET_201111_ROSE_LIFE + 2)  --ÐÞÕý·ÀÖ¹NPCÏûÊ§ÄÃ²»µ½×îºóÒ»´Î¾­Ñé
	else
    ...
    for i = 1, nTeamSize do
        ...
        VET_201111_ROSE_TaskGroup:SetTask(VET_201111_ROSE_TaskGroup.Time_End, GetTime() + VET_201111_ROSE_LIFE + 20)     --´¥·¢Æ÷ÖÕÖ¹Ê±¼ä£¬·ÀÖ¹NPCÒâÍâ¶ªÊ§¡£´¥·¢Æ÷½«ÔÚNPCÊ±ÏÞ10ÃëºóÏûÊ§
        ...
        gf_WriteLogEx(VET_201111_LOG_TITLE,VET_201111_TB_LOG_ACTION_LIST[2]) 					--by xiongyizhi ÐÂ¼Ó²Î¼Ó»î¶¯log
...
function TalkToCaptain()
	...
	if CheckToCaptain() == 1 then
		for i = 1, nTeamSize do
			PlayerIndex = GetTeamMember(i)
			if GetCaptainName() == GetName() then
        			Talk(1, "", "Thµnh viªn trong tæ ®éi b¹n ®· tham gia nhËn th­ëng 4 lÇn trong ngµy h«m nay ")
        			PlayerIndex = nOldPlayerIdx
        			return 1
    			end
		end
