Include("\\script\\online_activites\\201409\\3\\mission.lua")
...
function OnTimer()
	...
	local nMMap = GetGlbValue(HD_GV_ID[3]);
	--DebugOutput("HD_MISSION_ID[3] =", mf_GetMissionV(HD_MISSION_ID[3], 1, nMMap), mf_GetMissionV(HD_MISSION_ID[3], 2, nMMap), mf_GetMissionV(HD_MISSION_ID[3], 3, nMMap));
	local nTime = mf_GetMissionV(HD_MISSION_ID[3], 3, nMMap);
	if GetTime() - nTime > 1800 then
		EndMission();
		return 0;
	end
	local npcIndex = mf_GetMissionV(HD_MISSION_ID[3], 2, nMMap);
	if hd_IsActivate(npcIndex) ~= 1 then
		return 0;
	end
	local nStep = mf_GetMissionV(HD_MISSION_ID[3], 99, nMMap);
	mf_SetMissionV(HD_MISSION_ID[3], 99, nStep + 1, nMMap);
	DebugOutput("nStep =", nStep, HD_MISSION_ID[3]);
