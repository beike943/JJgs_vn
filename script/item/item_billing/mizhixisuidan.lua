...
function OnUse()
	Say("Sau khi dïng <color=yellow>MËt chÕ TÈy Tïy ®¬n<color> cã thÓ c¶i t¹o 5 ®iÓm n¨ng lùc c¬ b¶n, b¹n muèn c¶i t¹o n¨ng lùc c¬ b¶n nµo?",
		6,
...
function confirm_strength_5()
	...
	if (nCurStrength <= 1) then
		Say("Giê b¹n kh«ng cÇn sö dông mËt chÕ TÈy Tñy §¬n!", 0)
		return
	end
	...
	if (DelItem(2, 0, 1044, 1) == 1) then
		...
		Say("B¹n kh«ng hÒ cã <color=yellow> mËt chÕ TÈy Tñy §¬n<color>!")
	end
end;
