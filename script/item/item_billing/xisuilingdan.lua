...
function OnUse()
	Say("Sau khi dïng <color=yellow>TÈy Tñy Linh §¬n<color> cã thÓ kh«i phôc 1 n¨ng lùc tïy ı, b¹n muèn kh«i phôc n¨ng lùc nµo?",
		6,
...
function confirm_strength_all()
	...
	if (nCurStrength <= 1) then
		Say("HiÖn b¹n kh«ng cÇn dïng TÈy Tñy Linh §¬n!", 0)
		return
	end
	...
	if (DelItem(2, 0, 137, 1) == 1) then
		...
		Say("B¹n kh«ng mang theo <color=yellow>TÈy Tñy Linh §¬n<color>!", 0)
	end
end;
