------------------------------------------------------------
-- ÃØÖÆ°Ë±¦Ï´ËèÏÉµ¤µÄ½Å±¾ babaoxisuixiandan.lua
--
...
function OnUse()
	Say("Sau khi dïng <color=yellow>Bİ mËt chÕ B¸t B¶o TÈy Tñy Tiªn §¬n<color> cã thÓ t¹o l¹i toµn bé n¨ng lùc cña b¹n, b¹n x¸c nhËn muèn t¹o l¹i kh«ng?",
		2,
...
function change_all_points()
	Say("Qu¸ tr×nh t¹o l¹i sÏ hao phİ <color=yellow>Bİ mËt chÕ B¸t B¶o TÈy Tñy Tiªn §¬n<color> quİ gi¸, b¹n muèn t¹o l¹i kh«ng?",
		2,
...
function confirm_change_all_points()
	...
	if ((nCurStrength <= 1) and 
		...
		(nCurEnergy <= 1) and 
		(nCurObserve <= 1)) then
		Say("HiÖn t¹i b¹n kh«ng cÇn sö dông Bİ mËt chÕ B¸t B¶o TÈy Tñy Tiªn §¬n!", 0)
		return
	end
	...
	if (DelItem(2, 0, 1046, 1) == 1) then
		...
		Say("B¹n kh«ng hÒ cã <color=yellow>Bİ mËt chÕ B¸t B¶o TÈy Tñy Tiªn §¬n<color>!", 0)
	end
end;
