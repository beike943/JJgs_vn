--IBÎïÆ·Ê¦ÃÅÒ°ÍâbossÕÙ»½ÎïÆ·½Å±¾
--×÷Õß£ºÕÔ¹ó´º
--¿ªÊ¼Ê±¼ä£º2007-01-16
--½áÊøÊ±¼ä£º
--±¸×¢£ºÆÁ±ÎµØÍ¼£º×÷Õß:
--ÀÎ·¿,701¡¢702¡¢703
--¶ñÈË¹È,708
--ÀŞÌ¨,801,802,803,805,810,811
--Õ½³¡,807,809,881,882,883,884
--Ê¦ÃÅĞŞÁ¶µØÍ¼,812,813,814,815,816,817,422
--¶àÈËÀŞÌ¨,821,822,823,824,825,826
--×é¶ÓÀŞÌ¨,827,828,829,830,831,832,833,834
Include("\\script\\online\\zgc_public_fun.lua")
--======================================================ÊÀ½çÊı¾İ·â×°=================================================================================
--º¯ÊıÃû³Æ£ºÊÀ½çÊı¾İget
--¹¦        ÄÜ£º°´Î»È¡µÃ°ï»áÊÀ½çÊı¾İ
File_name = "\\script\\item\\item_billing\\wulinmiling.lua"
function Zgc_pub_getwrdtask(id,fun_id)
	SetTaskTemp(158,id)
	SetTaskTemp(160,fun_id)
	ApplyRelayShareData("Sè liÖu ID thÕ giíi",0,0,File_name,"CallBackGetWrdTask")
end
function CallBackGetWrdTask(szKey, nKey1, nKey2, nCount)
	local id = (GetTaskTemp(158)-1)
	local fun_id = tostring(GetTaskTemp(160))
	local suit_id = tostring(floor(id/10)+1)
	local wrd_data	= {}
	wrd_data[0],wrd_data[1],wrd_data[2],wrd_data[3],wrd_data[4],wrd_data[5],wrd_data[6],wrd_data[7],wrd_data[8],wrd_data[9] = GetRelayShareDataByKey(szKey, nKey1, nKey2,suit_id)
	local save_id = (id - (floor(id/10)*10))
	if wrd_data[save_id] == nil then
		wrd_data[save_id] = 0 
	end
	local return_fun = "WrdDataGetReturn_"..fun_id.."("..wrd_data[save_id]..")"
	dostring(return_fun)
end
--º¯ÊıÃû³Æ£ºÊÀ½çÊı¾İset
--¹¦        ÄÜ£º°´Î»ÉèÖÃÊÀ½çÊı¾İ
function Zgc_pub_setwrdtask(id,value,fun_id)
	SetTaskTemp(158,id)
	SetTaskTemp(159,value)
	SetTaskTemp(160,fun_id)
	ApplyRelayShareData("Sè liÖu ID thÕ giíi",0,0,File_name,"CallBackSetWrdTask")
end
function CallBackSetWrdTask(szKey, nKey1, nKey2, nCount)
	local id =(GetTaskTemp(158)-1)
	local value = GetTaskTemp(159)
	local fun_id = tostring(GetTaskTemp(160))
	local suit_id = tostring(floor(id/10) + 1)			--¼ÆËã´óÎ»ÖÃ
	local wrd_data	= {}
	wrd_data[0],wrd_data[1],wrd_data[2],wrd_data[3],wrd_data[4],wrd_data[5],wrd_data[6],wrd_data[7],wrd_data[8],wrd_data[9] = GetRelayShareDataByKey(szKey, nKey1, nKey2,suit_id)
	DelRelayShareDataCopy(szKey,nKey1,nKey2)
	for i = 0, 9 do
		if wrd_data[i] == nil then
			wrd_data[i] = 0 
		end
	end
	local save_id = (id - (floor(id/10)*10))
	wrd_data[save_id] = value
	local rtn_flag = AddRelayShareData("Sè liÖu ID thÕ giíi",0,0, File_name, "end_dialog", 0,suit_id, "dddddddddd", wrd_data[0],wrd_data[1],wrd_data[2],wrd_data[3],wrd_data[4],wrd_data[5],wrd_data[6],wrd_data[7],wrd_data[8],wrd_data[9])
	local return_fun = "WrdDataSetReturn_"..fun_id.."()"
	dostring(return_fun)
end
--=============================¹«¹²±äÁ¿¶¨Òå======================
	boss_list = {
		{},
		{},
		{},
		{"HuyÒn Ch©n","HuyÒn Ch©n",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\xuanzhen.lua"},			--bossÃû³Æ£¬Ä£°åÃû£¬¶ÔÓ¦ÎïÆ·ID£¬¶ÔÓ¦ÎïÆ·Ãû³Æ£¬Ã¿Ìì¿ÉÒÔÕÙ»½µÄ×î´óÊıÁ¿£¬NPC¹ÒµÄË÷Òı½Å±¾
		{"§­êng Hû","§­êng Hû",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\tangxi.lua"},
		{"TuÖ Minh","TuÖ Minh",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\huiming.lua"},
		{"Hoµng Chİnh NhÊt","Hoµng Chİnh NhÊt",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\huangzhengjian.lua"},
		{"Thanh Th­","Thanh Th­",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\qingshu.lua"},
		{"D­¬ng Diªn §øc","D­¬ng Diªn §øc",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\yangwulang.lua"},
		{"Cæ MÆc","Cæ MÆc",1015,"Vâ L©m bİ lÖnh",3,"\\script\\task\\faction_boss\\gumo.lua"},
	}
--================================Ö÷Âß¼­=========================
--**************************ÎïÆ·Ê¹ÓÃÖ÷º¯Êı************************
function OnUse(goods_index)
	SetItemUseLapse(id,90)
	if GetFightState() == 0 then
		Talk(1,"end_dialog","N¬i ®©y kh«ng ®­îc sö dông vËt phÈm nµy!")
		return
	end
	local MapID,Att_X,Att_Y = GetWorldPos()
	if MapID > 700 then
		Talk(1,"end_dialog","N¬i ®©y kh«ng ®­îc sö dông vËt phÈm nµy!")
		return
	end
	if GetPKEnmityState() > 0 then
		Talk(1,"end_dialog"," ë tr¹ng th¸i so tµi kh«ng cho phĞp dïng vËt phÈm nµy!")
		return
	end
	if city_map_chk() == 1 then
		return
	end
	Say("B¹n muèn dïng Vâ L©m bİ lÖnh triÖu gäi cao thñ m«n ph¸i?",
		8,
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån HuyÒn Ch©n [ThiÕu L©m]/#boss_create(4)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån §­êng Hû [§­êng M«n]/#boss_create(5)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån TuÖ Minh [Nga Mi]/#boss_create(6)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hånVµng Ch¸nh NhÊt [C¸i Bang]/#boss_create(7)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån Thanh Th­ [Vâ §ang]/#boss_create(8)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån D­¬ng Diªn §øc [D­¬ng M«n]/#boss_create(9)",
		"Ta muèn sö dông MËt lÖnh thiÕu l©m gäi hån Cæ M¹c [Ngò §éc]/#boss_create(10)",
		"§Ó ta suy nghÜ l¹i/end_dialog"
		)
end
--***************************boss´´½¨Í³Ò»º¯Êı·â×°*************************
function boss_create(boss_diff)
	SetTaskTemp(164,boss_diff)
	Zgc_pub_getwrdtask(11,11)
end
function WrdDataGetReturn_11(date_seq)
	local boss_diff = GetTaskTemp(164)
	if date_seq > zgc_pub_day_turn(1) then
		Talk(1,"end_dialog",boss_list[boss_diff][1].."cã chót nghi ngê, ngµy mai h·y quay l¹i!")
		return
	elseif date_seq == zgc_pub_day_turn(1) then
		Zgc_pub_getwrdtask(boss_diff,1)
	else
		Zgc_pub_setwrdtask(11,zgc_pub_day_turn(1),11)
	end
end
--*********************************bossÊıÁ¿ÉèÖÃ********************************
function WrdDataSetReturn_11()
	local boss_diff = GetTaskTemp(164)
	boss_num_set(boss_diff)
end
--*********************************bossÊıÁ¿»ØÖÃ********************************
function boss_num_set(boss_diff)
	SetTaskTemp(158,boss_diff)
	ApplyRelayShareData("Sè liÖu ID thÕ giíi",0,0,File_name,"CallBackBossNumSet")
end
function CallBackBossNumSet(szKey, nKey1, nKey2, nCount)			--bossÊıÁ¿ÉèÖÃ»Øµ÷
	DelRelayShareDataCopy(szKey,nKey1,nKey2)
	local boss_diff = GetTaskTemp(158)
	local boss_num = {}
	for i = 1,10 do
		if i == boss_diff then
			boss_num[i] = 1
		else
			boss_num[i] = 0
		end
	end
	AddRelayShareData("Sè liÖu ID thÕ giíi",0,0,File_name,"end_dialog",0,1,"dddddddddd",boss_num[1],boss_num[2],boss_num[3],boss_num[4],boss_num[5],boss_num[6],boss_num[7],boss_num[8],boss_num[9],boss_num[10])
	boss_crt_finish(boss_diff,0)
end
--*******************************Ò°ÍâbossÊıÁ¿ÅĞ¶Ï*******************************
function WrdDataGetReturn_1(boss_crt_num)
	local boss_diff = GetTaskTemp(164)
	if boss_crt_num >= boss_list[boss_diff][5] then
		Talk(1,"end_dialog",boss_list[boss_diff][1].."cã chót nghi ngê, ngµy mai h·y quay l¹i!")
		return
	else
		boss_crt_finish(boss_diff,boss_crt_num)
	end
end
--****************************ÕÙ»½bossÖ´ĞĞº¯Êı**********************************
function boss_crt_finish(boss_diff,boss_crt_num)
	if GetFightState() == 0 then
		Talk(1,"end_dialog","N¬i ®©y kh«ng ®­îc sö dông vËt phÈm nµy!")
		return
	end
	local MapID,Att_X,Att_Y = GetWorldPos()
	if MapID > 700 then
		Talk(1,"end_dialog","N¬i ®©y kh«ng ®­îc sö dông vËt phÈm nµy!")
		return
	end
	if GetPKEnmityState() > 0 then
		Talk(1,"end_dialog"," ë tr¹ng th¸i so tµi kh«ng cho phĞp dïng vËt phÈm nµy!")
		return
	end
	if city_map_chk() == 1 then
		return
	end
	if DelItem(2,1,boss_list[boss_diff][3],1) ~= 1 then
		Talk(1,"end_dialog","Xin x¸c nhËn trªn ng­êi cã <color=red>"..boss_list[boss_diff][4].."<color>!")
		return
	end
	local map_ID,att_X,att_Y = GetWorldPos()
	local npc_index = CreateNpc(boss_list[boss_diff][1],boss_list[boss_diff][2],map_ID,att_X,att_Y,-1,1,1,0)
	if boss_list[boss_diff][1] ~= "" then
		SetNpcScript(npc_index,boss_list[boss_diff][6])
	end
	Zgc_pub_setwrdtask(boss_diff,(boss_crt_num + 1),2)
end
--*************************bossÊıÁ¿Ôö¼Ó»Øµ÷º¯Êı£¨¿Õ£©******************************
function WrdDataSetReturn_2()
end
--2008-05-14Ôö¼Ó³ÇÊĞµØÍ¼²»ÄÜÊ¹ÓÃ
Tb_city_map_id = {100,150,200,300,350,400,500}
function city_map_chk()
	local map_id = GetWorldPos()
	for i = 1,getn(Tb_city_map_id) do
		if map_id == Tb_city_map_id[i] then
			Talk(1,"","<color=green>Nh¾c nhë<color>: kh«ng thÓ dïng vËt nµy trong thµnh!")
			return 1
		end
	end
	return 0
end

