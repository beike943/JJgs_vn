...
t_itemconfig = 
{
	...
	{2, 1, 30491, "千变金卷", 1000, 9999},
	{2, 1, 30492,"千变银卷", 100, 9999},
	{2, 1, 30493,"千变铜卷", 10, 9999},
}
...
function _do_use_item(nType, nItemIndex)
	...
	if t then
		...
		AskClientForNumber(format("_use_item%d", nType), 1,n, format("使用几个%s?",t[4]))
--		if t[6] > 1 then
--			AskClientForNumber(format("_use_item%d", nType), 1,t[6], format("使用几个%s?",t[4]))
--		else
...
function _real_use(nType, nItemIndex, nNum)
	...
	if t and nNum >= 1 and nNum <= t[6] then
		...
		Msg2Player(format("你使用了%d个%s，获得了%d御灵分", nNum, t[4], nGain))
