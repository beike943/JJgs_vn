...
function show_page(nPageNum,nRecordCount)
	...
	tinsert(selTab,"\n §ãng");
	Say("Xin chän mËt tÞch s­ m«n muèn ®æi!",getn(selTab),selTab);
end
...
function book_add(book_seq)
	...
	if add_flag == 1 then
		Msg2Player("Chóc mõng B¹n nhËn ®­îc 1 mËt tÞch s­ m«n!")
	else
		WriteLog(GetName()..":§æi mËt tÞch s­ m«n:"..way_book[book_seq][1].." thÊt b¹i, ký hiÖu:"..add_flag)
	end
end