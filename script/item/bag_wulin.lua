...
T_BOX={
	[200130575] = {T_WULING_FREE, T_WULING_IB,},--武林礼包
	[200130576] = {T_HAOXIA_FREE, T_HAOXIAO_IB},----武林豪侠礼包
}
...
function onUse_Confirm(nItemIndex, bConfirm)
	...
	if tCfg then
		if getn(tCfg) > 1 then
			Say(format("[%s]有%d什么办法？",GetItemName(nItemIndex),getn(tCfg)),
				3,
				format("\n免费/#onUse_Real(%d,%d,%d)", nItemIndex, 1, 0),
				format("\n打开/#onUse_Real(%d,%d,%d)", nItemIndex, 2, 0),
				"\n跳过/nothing");
			return
...
function onUse_Real(nItemIndex, nOpenIndex, bConfirm)
	...
	if tCfg then
		...
		if szConsume and 1~= bConfirm then
			...
			Say(format("同[%s]意<color=yellow>[%s]<color>,打开吗？",GetItemName(nItemIndex), szConsumeDesc),
			2,
			format("\n确认打开/#onUse_Real(%d,%d,%d)", nItemIndex, nOpenIndex, 1),
			"\n跳过/nothing");
			return
		end
		local szFunc = format("onInputNumCallBack_%d_%d",ID, nOpenIndex)
		local nMax = GetItemCount(nG,nD,nP)
		AskClientForNumber(szFunc, 1, nMax, "你想开多少个?")
	end
end
