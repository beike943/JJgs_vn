...
function SB_HandleExp(exp)
	...
	if expGained < SB_MAX_EXP then
		...
		Talk(1, "", "§¹i hiÖp ®· nhËn ®ñ "..SB_MAX_EXP.." ®iÓm kinh nghiÖm khi ®¸nh qu¸i!")
	end
end
...
function SB_OnEntered()
	SB_LearnSkill();
	SB_LockEquipment(1);
	gf_WriteLogEx("Hoat dong thang 12 n¨m 2010", "B¸o danh tham gia ®¸nh tuyÕt");
end
...
function SB_Announce()
	AddGlobalNews("Thiªn S¬n TuyÕt Yªu dÉn theo tiÓu qu¸i ®· xuÊt hiÖn t¹i §Êu Tr­êng TuyÕt ®¹i n¸o linh ®×nh, c¸c vÞ anh hïng hao kiÖt h·y nhanh chãng ®Õn t×m Xa Phu §Êu Tr­êng TuyÕt t¹i Thµnh §« ®Ó chuyÓn vµo ®Êu tr­êng tuyÕt thu phôc bän chóng");	
end
...
function SB_SpawnMonsters(mapID, x, y, monsterTemplate, monsterName, deathScript, monsterCount)
	...
	for j=1, monsterCount do
		if CountGroupUnit(SB_GROUP_MOB) < SB_MAX_NPC then	
			...
			Talk(1, "", "Sè qu¸i vËt ®· qu¸ ®«ng vµ hung h·n, h·y tiªu diÖt hÕt chóng!")
			break
		end
	end	
end
...
function SB_OnMonsterDied()
	...
	if killedCount >= SB_MONSTER_COUNT_PER_WAVE then
		Msg2MSAll(SB_MISSION_ID, "Qu¸i vËt ®· bÞ tiªu diÖt toµn bé, cã thÓ khiªu chiÕn ®ît qu¸i tiÕp theo");
	end
end
