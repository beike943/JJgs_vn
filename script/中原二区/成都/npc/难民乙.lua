...
-- ½£ÏÀÇéÔµonlineII ¶ëÃ¼ÃÅÅÉÈÎÎñÄÑÃñÒÒ½Å±¾ÎÄ¼ş
-- Edited by peres
-- 2005/02/17 PM 18:03
...
function main ()
    ...
    local nStep = GetTask(TASK_JJC_YI);
    ...
    UWorld1506 = GetTask(1506)
  	UWorld1514 = GetTask(1514)
  	if ( UWorld1506 >= 5   and  UWorld1514 == 0 )   then
...
local nTaskValue = FN_GetTaskState(FN_EM);
	...
	if (nTaskValue==2) then
		if (GetTask(EM_MANTALK_02)==0) then
			task_006();
			return
		else
			Say("§a t¹ §¹i hiÖp!",0);
