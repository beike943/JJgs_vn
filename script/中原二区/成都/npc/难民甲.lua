...
-- ½£ÏÀÇéÔµonlineII ¶ëÃ¼ÃÅÅÉÈÎÎñÄÑÃñ¼×½Å±¾ÎÄ¼ş
-- Edited by peres
-- 2005/02/17 PM 18:03
...
function main (bTag)
	...
	if nTaskValue == 2 and nStep <= 5 and nStep >= 2 then
	    local szSay = {
	...
	UWorld1513 = GetTask(1513)
	if ( UWorld1506 >= 5   and  UWorld1513 == 0 )   then
		...
		if UWorld1506 == 9 then 
			--AddNote("ÄÑÃñÍ¨ÖªÍê³É£¬»Ø¼ûÖÜÉÆÈË£¡")
...
local nTaskValue = FN_GetTaskState(FN_EM);
	...
	if (nTaskValue==2) then
		...
		if (GetTask(EM_MANTALK_01)==0) then
			task_005();
			return
		else
			Say("§a t¹ §¹i hiÖp!",0);
...
function  zaimin1000_yes()
	Talk(1,"","Bå T¸t chuyÓn thÕ cøu chóng ta råi!")
	Pay(1000)
	TaskTip("B¹n bè thİ cho N¹n d©n 1000 tiÒn ®ång!")
end;

function  zaimin500_yes()
	Talk(1,"","Trêi! Ta kh«ng ph¶i ®ang n»m m¬ chø?")
	Pay(500)
