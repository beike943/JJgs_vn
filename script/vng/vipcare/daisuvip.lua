...
szNPCname = "<color=green>ßπi s¯ Vip: <color>"
szLogTitle = "VIPCARE"
szLogAction = "nhÀn"
...
function main()
	...
	--if nTuan > 0 then
	--tinsert(tbSay, "NhÀn th≠Îng Tu«n ".. (tbVipTitle[nTuan][1] or "") .."/ThuongTuan_Award")
	--end
	
	if nThang > 0 then
		tinsert(tbSay, "NhÀn th≠Îng Th∏ng ".. (tbVipTitle[nThang][1] or "") .."/ThuongThang_Award")
	end
	--if nQui > 0 then
	--	tinsert(tbSay, "NhÀn th≠Îng Qu› ".. (tbVipTitle[nQui][1] or "") .."/ThuongQui_Award")
	--end
	tinsert(tbSay, "Ta chÿ gh– xem/gf_DoNothing")
	Say(szNPCname.."Ta chﬁu tr∏ch nhi÷m trao quµ th≠Îng cho c∏c nh©n s‹ Vip. N’u c∏c hπ nªm trong danh s∏ch Vip h∑y Æ’n g∆p ta.", getn(tbSay), tbSay)
end
...
--		Talk(1,"",szNPCname.."C∏c hπ c«n "..tbAwardSet[nBonus][2].." ´ hµnh trang, "..tbAwardSet[nBonus][3].." s¯c l˘c Æ” nhÀn ph«n th≠Îng nµy!");
--		return 0
--	end
--	if ThuongTuan_Set() == 1 then
--		LIB_Award:Award(tbAwardSet[nBonus][1])
--		Talk(1,"",szNPCname.."C∏c hπ Æ∑ nhÀn thµnh c´ng ph«n th≠Îng tu«n <bclr=".. tbVipTitle[nBonus][2]  ..">".. tbVipTitle[nBonus][1] .."<bclr>.")
...
function ThuongThang_Award()
	...
	item = {
					...
					{ gdp = {2, 1, 30523, 12}, name= "C p 3 T»y T©m Thπch" },
					{ gdp = {2, 1, 30491, 1},   name= "V– Ti”u Y Vµng" },					 
				},
	 }
	local tbAward2 = { nBachCau3 = 120*60, nLucThan3 = 120*60, nTamThanh3 = 120*60, nTuLinh3 = 120*60,nGold = 13000000,
	 item = { 
	 			{gdp ={2, 1, 30492, 2}, name = "V– Ti”u Y Bπc" }
	 		}
	 }
	...
	if ThuongThang_Set() == 1 then
		LIB_Award:Award(tbAwardSet[nBonus][1])
		Talk(1,"",szNPCname.."C∏c hπ Æ∑ nhÀn thµnh c´ng ph«n th≠Îng th∏ng <bclr=".. tbVipTitle[nBonus][2]  ..">".. tbVipTitle[nBonus][1] .."<bclr>.")
...
--			{gdp={2,1,30536,30}, name="T»y T©m Thπch Kho∏ng"},
--			{gdp={2,1,30071,200}, name="Nh©n S©m Vπn N®m"},
--		},
--		nGold = 30000000,
--	}
--	local tbAward3 = {
...
--			{gdp={2,1,30523,3}, name="C p 3 T»y T©m Thπch"},
