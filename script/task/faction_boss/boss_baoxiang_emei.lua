-- =============================================================
-- FileName : boss_baoxiang_emei.lua
-- Desc     : ¶ëáÒboss±¦ÏäµÄ½Å±¾£¬ÃÅÅÉbossÈÎÎñÖÐµôÂäµÄ±¦Ïä£¬µãÈ¡¿ÉÒÔ»ñµÃºÃ¶àgood¶«Î÷µÄ¹¦ÄÜ
-- Date     : Dec 19 2006
...
function main()
	...
	if nRand <= 20 then
		local nRandIdx = random(1,getn(tGaojiMiji[7]));
		AddItem(tGaojiMiji[7][nRandIdx][2],tGaojiMiji[7][nRandIdx][3],tGaojiMiji[7][nRandIdx][4],1);
		Msg2Player("ThiÕu hiÖp më b¶o r­¬ng nhËn ®­îc "..tGaojiMiji[7][nRandIdx][1]);
	else
	...
	Msg2SubWorld("Ng­êi ch¬i"..GetName().."§¸nh b¹i HuÖ Minh, nhËn ®­îc Tói tïy th©n");
	-- °Ñ×Ô¼ºÉ¾³ý
	local nSelfIndex = GetTargetNpc()
	SetNpcLifeTime(nSelfIndex, 0)
	SetTask(TASK_ID_HAVE_GOT_TODAY, 0)
	WriteLogEx("Boss su mon","nhÆt tói Nga My")
end;
...
function get_suit_3_head()
	...
	elseif (nSel <= 42) then		-- ¶ëáÒ·ð¼Ò×Ô¼ºµÄ¸ÅÂÊ±È½Ï¸ß
		if (nBodyType > 2) then
			nItemDetail = 820 + (nBodyType - 2)
		else
			nItemDetail = 820 + random(1, 2)
		end
	elseif (nSel <= 68) then		-- ¶ëáÒË×¼Ò×Ô¼ºµÄ¸ÅÂÊ±È½Ï¸ß
		if (nBodyType > 2) then
