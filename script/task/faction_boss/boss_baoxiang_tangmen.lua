...
-- Desc     : ÌÆÃÅboss±¦ÏäµÄ½Å±¾£¬ÃÅÅÉbossÈÎÎñÖÐµôÂäµÄ±¦Ïä£¬µãÈ¡¿ÉÒÔ»ñµÃºÃ¶àgood¶«Î÷µÄ¹¦ÄÜ
-- Date     : Dec 19 2006
...
function main()
	...
	if nRand <= 20 then
		...
		Msg2Player("ThiÕu hiÖp më b¶o r­¬ng nhËn ®­îc "..tGaojiMiji[5][nRandIdx][1]);
	else
	...
	Msg2SubWorld("Ng­êi ch¬i"..GetName().."§¸nh b¹i §­êng Hû, nhËn ®­îc Tói tïy th©n");
	-- °Ñ×Ô¼ºÉ¾³ý
	local nSelfIndex = GetTargetNpc()
	SetNpcLifeTime(nSelfIndex, 0)
	SetTask(TASK_ID_HAVE_GOT_TODAY, 0)
	WriteLogEx("Boss su mon","nhÆt tói §­êng M«n")
end;
...
function get_suit_3_head()
	...
	elseif (nSel <= 60) then		-- ÌÆÃÅ×Ô¼ºµÄ¸ÅÂÊ±È½Ï¸ß		
