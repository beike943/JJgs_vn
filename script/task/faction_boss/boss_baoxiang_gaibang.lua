...
-- Desc     : Ø¤°ïboss±¦ÏäµÄ½Å±¾£¬ÃÅÅÉbossÈÎÎñÖÐµôÂäµÄ±¦Ïä£¬µãÈ¡¿ÉÒÔ»ñµÃºÃ¶àgood¶«Î÷µÄ¹¦ÄÜ
-- Date     : Dec 19 2006
...
function main()
	...
	if nRand <= 20 then
		...
		Msg2Player("ThiÕu hiÖp më b¶o r­¬ng nhËn ®­îc "..tGaojiMiji[10][nRandIdx][1]);
	else
	...
	Msg2SubWorld("Ng­êi ch¬i"..GetName().."§¸nh b¹i Hoµng ChÝnh NhÊt, nhËn ®­îc Tói tïy th©n");
	-- °Ñ×Ô¼ºÉ¾³ý
	local nSelfIndex = GetTargetNpc()
	SetNpcLifeTime(nSelfIndex, 0)
	SetTask(TASK_ID_HAVE_GOT_TODAY, 0)
	WriteLogEx("Boss su mon","nhÆt tói C¸i Bang")
end;
...
function get_suit_3_head()
	...
	elseif (nSel <= 50) then		-- Ø¤°ï¾»ÒÂ×Ô¼ºµÄ¸ÅÂÊ±È½Ï¸ß
		nItemDetail = 1120 + nBodyType
	elseif (nSel <= 76) then		-- Ø¤°ïÎÛÒÂ×Ô¼ºµÄ¸ÅÂÊ±È½Ï¸ß
