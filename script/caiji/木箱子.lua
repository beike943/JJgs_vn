...
nTotalItemCount = 7					-- ×Ü¹²ÓĞ¶àÉÙ²É¼¯ÎïÆ·

szItemGenre = 		{2,2,2,2,2,2,2}			-- ²É¼¯ÎïÆ·µÄ´óÀà
szItemDetail = 		{1,1,1,1,1,1,1}			-- ²É¼¯ÎïÆ·µÄ¸±Àà
szItemParticular = 	{908,909,910,911,913,914,915}			-- ²É¼¯ÎïÆ·µÄÏêÏ¸Àà±ğ
szItemGailv = 		{15,30,44,58,72,86,100}		-- ²É¼¯ÎïÆ·µÄ¸ÅÂÊ£¨ÕâÀïÌîµÄÊ±ºò£¬ºóÃæÒ»¸öÒª¼ÓÉÏÇ°ÃæÒ»¸öµÄ¸ÅÂÊ£©
szItemLowCount =	{20,20,20,20,20,20,20}		-- ²É¼¯ÎïÆ·×îµÍÊıÁ¿
szItemHighCount =	{40,40,40,40,40,40,40}		-- ²É¼¯ÎïÆ·×î¸ßÊıÁ¿

nSkillType = 0						-- ²É¼¯ÀàÉú»î¼¼ÄÜ´óÀàĞÍ
nSkillID = 7						-- ¼¯Áé¼¼ÄÜID
nSkillLevelLow = 20					-- ĞèÒª´ïµ½µÄ²É¼¯¼¼ÄÜ×îµÍµÈ¼¶
nSkillLevelHigh = 30				-- ĞèÒª´ïµ½µÄ²É¼¯¼¼ÄÜ×î¸ßµÈ¼¶

nDigToolCount = 2							-- ²É¼¯¹¤¾ßÖÖÀàÊıÁ¿
nDigToolGenre = {0, 0}				-- ²É¼¯¹¤¾ß´óÀà
nDigToolDetail = {200, 200}				-- ²É¼¯¹¤¾ß¸±Àà
nDigToolParticular = {36, 38}		-- ²É¼¯¹¤¾ßÏêÏ¸Àà
...
function OnProgressCallback()
	...
	-- ÌáÊ¾
	if (nChoice == 1) then
		Say("VËt liÖu linh phï nµy cã thÓ dïng, ng­¬i t×m ®­îc"..nSelCount.." Vâ §ang phï th¹ch!", 0)
	elseif (nChoice == 2) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." C¸i Bang phï th¹ch!", 0)
	elseif (nChoice == 3) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." ThiÕu L©m phï th¹ch!", 0)
	elseif (nChoice == 4) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." Nga My phï th¹ch!", 0)
	elseif (nChoice == 5) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." §­êng M«n phï th¹ch!", 0)
	elseif (nChoice == 6) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." Ngò §éc phï th¹ch!", 0)
	elseif (nChoice == 7) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." Thiªn Ba D­¬ng phñ phï th¹ch!", 0)
	else
