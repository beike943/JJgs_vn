...
nTotalItemCount = 6					-- ×Ü¹²ÓĞ¶àÉÙ²É¼¯ÎïÆ·

szItemGenre = 		{2,2,2,2,2,2}			-- ²É¼¯ÎïÆ·µÄ´óÀà
szItemDetail = 		{1,1,1,1,1,1}			-- ²É¼¯ÎïÆ·µÄ¸±Àà
szItemParticular = 	{901,902,903,904,906,907}			-- ²É¼¯ÎïÆ·µÄÏêÏ¸Àà±ğ
szItemGailv = 		{17,34,52,68,84,100}		-- ²É¼¯ÎïÆ·µÄ¸ÅÂÊ£¨ÕâÀïÌîµÄÊ±ºò£¬ºóÃæÒ»¸öÒª¼ÓÉÏÇ°ÃæÒ»¸öµÄ¸ÅÂÊ£©
szItemLowCount =	{20,20,20,20,20,20}		-- ²É¼¯ÎïÆ·×îµÍÊıÁ¿
szItemHighCount =	{40,40,40,40,40,40}		-- ²É¼¯ÎïÆ·×î¸ßÊıÁ¿

nSkillType = 0						-- ²É¼¯ÀàÉú»î¼¼ÄÜ´óÀàĞÍ
nSkillID = 7						-- ¼¯Áé¼¼ÄÜID
nSkillLevelLow = 1					-- ĞèÒª´ïµ½µÄ²É¼¯¼¼ÄÜ×îµÍµÈ¼¶
nSkillLevelHigh = 10				-- ĞèÒª´ïµ½µÄ²É¼¯¼¼ÄÜ×î¸ßµÈ¼¶
...
function OnProgressCallback()
	...
	-- ÌáÊ¾
	if (nChoice == 1) then
		Say("VËt liÖu linh phï nµy cã thÓ dïng, ng­¬i t×m ®­îc"..nSelCount.." BiÖn Kinh phï th¹ch!", 0)
	elseif (nChoice == 2) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." Thµnh §« phï th¹ch!", 0)
	elseif (nChoice == 3) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." TuyÒn Ch©u phï th¹ch!", 0)
	elseif (nChoice == 4) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." T­¬ng D­¬ng phï th¹ch!", 0)
	elseif (nChoice == 5) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." §¹i Lı phï th¹ch!", 0)
	elseif (nChoice == 6) then
		Say("Kh«ng ngê bªn trong cßn mét sè ®å, b¹n nhËn ®­îc "..nSelCount.." D­¬ng Ch©u phï th¹ch!", 0)
	else
