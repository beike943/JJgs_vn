...
Item_Table = {2,1,30392,"L≠¨ng S¨n t›n vÀt"};
ITEM_NEED_NUM = 9;
...
function OnUse(nItem)
	...
	if GetItemCount(Item_Table[1],Item_Table[2],Item_Table[3]) < ITEM_NEED_NUM then
		Talk(1,"",format("SË l≠Óng L≠¨ng S¨n t›n vÀt kh´ng ÆÒ %d ",ITEM_NEED_NUM));
	end
	--ºÏ≤Èµ±ÃÏ π”√¥Œ ˝
	if LSB_MailUseTimes() >= MAX_USE_TIMES then
		Talk(1,"",format("MÁi ngµy sˆ dÙng tËi Æa %d l«n L≠¨ng S¨n t›n vÀt",MAX_USE_TIMES));
		return 0;
	end
	--µ±ÃÏµÿÕº «∑Ò∫œ  
