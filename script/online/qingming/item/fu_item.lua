...
--½Å±¾¹¦ÄÜ£º¼ÀÆ··ûÖ½Ê¹ÓÃ½Å±¾£¬¿ÉÒÔÈÃÍæ¼ÒÔÚÒ»ÌìÄÚ¶à½ÓÁ½´ÎÇåÃ÷ÈÎÎñ¾íÖáÈÎÎñ
--²ß»®ÈË£º°¢ËÉ
--´úÂë¿ª·¢ÈË£º´å³¤
...
tomb_sweep_2007_fu_use_date_seq	= 859				--¼ÀÆ··ûÖ½µÄÊ¹ÓÃÈÕÆÚĞòÊı
tomb_sweep_2007_fu_use_num = 860					--¼ÀÆ··ûÖ½½ñÌìµÄÊ¹ÓÃÊıÁ¿
...
function OnUse(goods_index)
	...
	elseif date_chk == -1 then
		Talk(1,"","<color=green>Nh¾c nhë<color>: VËt phÈm chØ dïng ®­îc trong thêi gian diÔn ra ho¹t ®éng TiÕt Thanh Minh <color=yellow>30/3/2007 - 8/4/2007<color>")
		return
	end
	--Ê¹ÓÃÊıÁ¿ÅĞ¶Ï
	...
	if (use_date_seq >= date_seq_now and use_num > 1) or date_seq_now < ((use_date_seq + task_deal_seq)/2) then
		Talk(1,"","<color=green>Nh¾c nhë<color>: VËt phÈm nµy mçi ngµy chØ dïng ®­îc <color=yellow>2<color> lÇn!")
		return
	end
	--ÎïÆ·É¾³ı¼ì²â
	...
	if use_num_dealed == 2 then
		...
		dia_use_num_remain = "Cßn sö dông ®­îc <color=yellow>"..(2-use_num_dealed).."<color> lÇn"
	end
