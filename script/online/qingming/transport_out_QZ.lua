...
LeavePointTab = {	--¼ÇÂ¼´ÓÁêÄ¹³öÀ´Ê±µÄ×ø±ê
			[1] = {100,1453,2812},	--ÈªÖİ¡¡ÖÜÏÀÚ£
			[2] = {200,1170,2834},	--ãê¾©¡¡ÖÜÏÀÚ£
...
function get_teel_task_prize()
	...
	if reel_task_date_seq_save >= date_seq_now and reel_task_num_save == 0 then
		Talk(1,"","MËt ®å Thanh Minh mçi ngµy chØ lµm ®­îc <color=yellow>1<color> lÇn, b¹n cã thÓ d¹o phè mua <color=yellow>giÊy cóng tÕ<color> t¨ng sè lÇn hoµn thµnh!")
		return
	end
	...
	if GetItemCount(2,0,(tomb_sweep_2007_start_goodsID + task_diff -1)) < 1 then
		Talk(1,"","Ng­¬i kh«ng mang theo sæ ghi chĞp nhiÖm vô µ?")
		return
	end
	if task_reel_table[task_diff][1] == 1 then			--É±¹ÖÈÎÎñ
		if GetTask(tomb_sweep_2007_task_reel_step) ~= 1 then
			Talk(1,"","<color=green>Chu gia nh©n<color>:"..Zgc_pub_sex_name().."kh«ng ®­îc nãi bËy tr­íc mé tiªn nh©n, lµm xong nhiÖm vô míi ®Õn ®©y!")
			return
		end
	else
		--ÎïÆ·ÓµÓĞ¼ì²â
		for i = 1,task_reel_table[task_diff][2] do
			...
			if task_reel_table[task_diff][10+add_num] ~= 1 then
				if GetItemCount(task_reel_table[task_diff][7+add_num],task_reel_table[task_diff][8+add_num],task_reel_table[task_diff][9+add_num]) < task_reel_table[task_diff][10+add_num] then
					Talk(1,"","Xin x¸c nhËn ng­¬i cã mang theo ®ñ <color=yellow>"..task_reel_table[task_diff][6+add_num].."<color>!")
					return
				end
			else
				if GetItemCount(task_reel_table[task_diff][7+add_num],task_reel_table[task_diff][8+add_num],task_reel_table[task_diff][9+add_num]) < task_reel_table[task_diff][10+add_num] then
				...
				elseif GetItemCount(task_reel_table[task_diff][7+add_num],task_reel_table[task_diff][8+add_num],task_reel_table[task_diff][9+add_num]) > task_reel_table[task_diff][10+add_num] then
					Talk(1,"","Ng­¬i cã mang"..task_reel_table[task_diff][6+add_num].."d­ ra <color=yellow>1<color> c¸i, l·o phu kh«ng thÓ nhËn.")
					return
				end
			end
		end
...
function prize_exp_add()
	...
	if DelItem(2,0,(tomb_sweep_2007_start_goodsID + task_diff -1),1) ~= 1 then
		Talk(1,"","Nhí mang theo sæ ghi chĞp nhiÖm vô nhĞ!")
		return
	end
	--½¡¿µ¾­Ñé×ª¾­Ñé
	local player_level = GetLevel()
...
function teel_task_release()
	...
	elseif task_diff >= 1 and task_diff <= 10 then
		Say("QuyÓn sæ nµy chİnh lµ"..Zgc_pub_sex_name().."ng­êi h÷u duyªn míi cã, ng­¬i thËt sù muèn hñy kh«ng?",
		2,
...
function tomb_sweep_2007_inf()
	...
	"\n Thêi gian ho¹t ®éng tõ <color=yellow>30/3 - 8/4<color>. \n Trong TiÕt Thanh Minh viÖc t¶o mé lµ chñ yÕu, ta cÇn ®Õn cóng b¸i <color=yellow>Chu ®¹i hiÖp, Hµn ®¹i hiÖp, Hiªn Viªn ®¹i hiÖp<color>.",
	"\n Trong TiÕt Thanh Minh mçi ngµy ®Õn bÊt kú mé phÇn cóng b¸i <color=yellow>1<color> lÇn b»ng c¸ch <color=yellow>v¸i l¹y<color>"..Zgc_pub_sex_name().."thµnh t©m nhËn ®­îc mËt ®å nhiÖm vô cña thñ l¨ng nh©n."
	)
end
function tomb_sweep_2007_inf_1()
	Talk(2,"","\n Hoµn thµnh nhiÖm vô <color=yellow>Hµn gia nh©n<color>. Mang <color=yellow>mËt ®å<color> ®Õn <color=yellow>Hµn gia nh©n<color> nhËn <color=yellow>phÇn th­ëng kinh nghiÖm<color>.",
		"\n Hoµn thµnh <color=yellow>15<color> lÇn nhiÖm vô <color=yellow>t¶o mé<color> sÏ nhËn ®­îc gÊp <color=yellow>10 lÇn<color> phÇn th­ëng."
)
end