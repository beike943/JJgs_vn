...
function OnUse()
	...
	if nMapID >= 700 then
		Talk(1,"","N¬i nµy hiÖn kh«ng thÓ sö dông TiÓu b¹ch tr­ phï");
		return 0;
	end;
	local selTab = {
				"§óng vËy!/confirm",
	...
	Say("Muèn sö dông TiÓu b¹ch tr­ phï kh«ng? Sau khi x¸c nhËn, b¹n sÏ mÊt thó nu«i hiÖn t¹i.",getn(selTab),selTab);
end;
...
function create_pet(sName)
	...
	local sPetName = "";
	if nSex == 1 then
		sPetName = "TiÓu B¹ch tr­ (mÑ)";
	else
		sPetName = "TiÓu B¹ch tr­ (bè)";
	end
