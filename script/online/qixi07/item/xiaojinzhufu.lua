...
function OnUse()
	...
	if nMapID >= 700 then
		Talk(1,"","N¬i nµy hiÖn kh«ng thÓ sö dông TiÓu kim tr­ phï");
		return 0;
	end;
	local selTab = {
				"§óng vËy!/confirm",
	...
	Say("Muèn sö dông TiÓu kim tr­ phï kh«ng? Sau khi nhÊp chuét x¸c nhËn, b¹n sÏ mÊt thó nu«i hiÖn t¹i.",getn(selTab),selTab);
end;
...
function create_pet(sName)
	...
	local sPetName = "";
	if nSex == 1 then
		sPetName = "TiÓu Kim tr­ (mÑ)";
	else
		sPetName = "TiÓu Kim tr­ (bè)";
	end
