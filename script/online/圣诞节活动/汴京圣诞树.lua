...
function give_present()
	local nPresentRemain = GetGlbValue(ID_PRESENT_NUMBER_BJ)
	local nEmpty = 0
	if(nPresentRemain > 0) then
		...
		if nRetCode == 0 then	--Ã»ÓĞ³É¹¦Ìí¼Ó
			...
			SetGlbValue(ID_PRESENT_NUMBER_BJ,nPresentRemain-1)
			SetTask(TASK_GOT_PRESENT,1) 
			SetTask(TASK_GOT_DATE,GetCurDate())
			if GetGlbValue(ID_PRESENT_NUMBER_BJ) == 0 then
				nEmpty = 1;
			end
		end
	else
	...
	if nEmpty == 1 then
		AddGlobalNews("Quµ c©y th«ng Gi¸ng Sinh ë Nam BiÖn Kinh ®· bŞ lÊy hÕt, ng­êi ch¬i nµo ch­a lÊy ®­îc h·y ®Õn lÇn sau.")
		Msg2SubWorld("Quµ c©y th«ng Gi¸ng Sinh ë Nam BiÖn Kinh ®· bŞ lÊy hÕt, ng­êi ch¬i nµo ch­a lÊy ®­îc h·y ®Õn lÇn sau.")
	end
end
