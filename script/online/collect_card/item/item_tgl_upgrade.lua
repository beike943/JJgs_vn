...
function OnUse_1(nItem)
	...
	if nRoute ~= 23 and nRoute ~= 29 and nRoute ~= 30 then
		...
		if DelItemByIndex(nItem, 1) == 1 then
			gf_AddItemEx2({2, 95, 204, CC_TGL_COST_NUM}, "Thiªn Cang LÖnh", CC_LOG_TITLE, "Sö dông LÔ Bao N©ng CÊp Thiªn C­¬ng LÖnh", 0, 1);
		end
		return 0;
	else
		--ÐÂÁ÷ÅÉ
		if DelItemByIndex(nItem, 1) == 1 then
			gf_AddItemEx2({2, 95, 204, CC_TGL_UPGRADE_NUM}, "Thiªn Cang LÖnh", CC_LOG_TITLE, "Sö dông LÔ Bao N©ng CÊp Thiªn C­¬ng LÖnh", 0, 1);
			gf_Modify("Exp", CC_TGL_UPGRADE_EXP);
			Msg2Global(format("Chóc mõng ng­êi ch¬i %s më %s nhËn ®­îc %s + %d", GetName(), "LÔ bao Thiªn Cang LÖnh N©ng CÊp", "Thiªn Cang LÖnh", CC_TGL_UPGRADE_NUM));
		end
		return 0;
	end
end
