...
function OnUse()
	...
	Say("Ng­¬i x¸c nhËn cã muèn dïng R­îu tÕ lÔ kh«ng? Dïng vËt phÈm nµy cã thÓ t¨ng <color=yellow>10 lÇn<color> kinh nghiÖm!",2,selTab)
end
--****************************¼ÀÆ·ÀÏ¾ÆµÄÊ¹ÓÃ***************************
function use()
	--Ê±¼äÏŞÖÆ
	...
	if kotow_date_seq >= zgc_pub_day_turn(1) then		
		Talk(1,"","H«m nay ng­¬i ®· cóng 1 lÇn, kh«ng thÓ dïng <color=yellow>R­îu tÕ lÔ<color>, ngµy mai <color=yellow>dïng<color>tr­íc khi cóng, dïng vËt phÈm nµy cã thÓ t¨ng 10 lÇn kinh nghiÖm.")
		return
	end
	--´ÎÊıÏŞÖÆ
	...
	if kotow_goods_diff == 8 or kotow_goods_diff == 6 or kotow_goods_diff == 9  or kotow_goods_diff == 11 then
		Talk(1,"","<color=yellow>R­îu tÕ lÔ<color> mçi ngµy chØ cã thÓ dïng 1 lÇn!")
		return
	end
	--ÎïÆ·É¾³ı¼ì²â
	if DelItem(2,0,389,1) ~= 1 then
		Talk(1,"","X¸c nhËn ng­¬i cã mang theo <color=yellow>R­îu tÕ lÔ<color>!")
		return
	end
	--Ê¹ÓÃÍê³É²Ù×÷
	...
	Talk(1,"","Ng­¬i ®· dïng <color=yellow>R­îu tÕ lÔ<color>, h«m nay cóng x¸c suÊt nhËn ®­îc kinh nghiÖm giang hå t¨ng.")
end
