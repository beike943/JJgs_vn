...
function OnUse()
	Say("Më r­¬ng ®ång Giang S¬n ph¶i cã ch×a khãa ®ång Giang S¬n, më kh«ng? ",
		2,
		"§ång ý/confirm_open",
		"Kh«ng cã g×/nothing")
end
...
function confirm_open()
	if GetItemCount(2,0,1017) < 1 then
		Talk(1,"","Më r­¬ng ®ång Giang S¬n ph¶i cã ch×a khãa ®ång Giang S¬n, b¹n kh«ng cã ch×a khãa ®ång Giang S¬n!");
		return
	else 
		...
		if DelItem(2,0,1026,1) == 1 and DelItem(2,0,1017,1) == 1 then
			...
			Msg2Player("Chóc mõng b¹n nhËn ®­îc "..tBoxAward[2][1][1]..tBoxAward[2][1][5].."!");
			...
			if nRandTwo <= tBoxAward[2][3][6] then
				...
				Msg2Player("Chóc mõng b¹n nhËn ®­îc "..tBoxAward[2][3][1]..tBoxAward[2][3][5].."!");
				Msg2Global("Ng­êi ch¬i"..GetName().."Më r­¬ng ®ång Giang S¬n nhËn ®­îc "..tBoxAward[2][3][1]..tBoxAward[2][3][5].."!");
			end
			if nRandThree <= tBoxAward[2][4][6] then
				AddItem(tBoxAward[2][4][2],tBoxAward[2][4][3],tBoxAward[2][4][4],tBoxAward[2][4][5],1);
				Msg2Player("Chóc mõng b¹n nhËn ®­îc "..tBoxAward[2][4][1]..tBoxAward[2][4][5].."!");
			end
			if nRandFour <= tBoxAward[2][5][2] then
				...
				if nRetCode == 1 then
					...
					Msg2Global("Ng­êi ch¬i"..GetName().."Më r­¬ng ®ång Giang S¬n nhËn ®­îc trang bÞ Hoµng §Õ "..tbHuangDiRing1[nRoute][nRand][1].."!");
					WriteLog("[07 Tam tiÕt liªn th«ng]:"..GetName().."Më r­¬ng ®ång Giang S¬n nhËn ®­îc trang bÞ Hoµng §Õ "..tbHuangDiRing1[nRoute][nRand][1]);
				else
					WriteLog("[B¸o lçi Tam tiÕt liªn th«ng 07]:"..GetName().."Më r­¬ng ®ång Giang S¬n nhËn ®­îc trang bÞ Hoµng §Õ "..tbHuangDiRing1[nRoute][nRand][1].."B¸o lçi khi AddItem, nRetCode:"..nRetCode);
				end				
			end
		end
	end
