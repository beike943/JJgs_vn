...
function confirm_open()
	if GetItemCount(2,0,1016) < 1 then
		Talk(1,"","Më r­¬ng s¾t Giang S¬n ph¶i cã ch×a kh¸c s¾t Giang S¬n, b¹n kh«ng cã ch×a kh¸c s¾t Giang S¬n!");
		return
	else 
		if Zgc_pub_goods_add_chk(5,35) ~= 1 then   --¿Õ¼ä¸ºÖØ¼ì²â
			return
		end	
		if DelItem(2,0,1025,1) == 1 and DelItem(2,0,1016,1) == 1 then
			AddItem(tBoxAward[1][1][2],tBoxAward[1][1][3],tBoxAward[1][1][4],tBoxAward[1][1][5],1);
			Msg2Player("Chóc mõng b¹n nhËn ®­îc "..tBoxAward[1][1][1]..tBoxAward[1][1][5].."!");
			local nRandOne = random(1,100)
			if nRandOne <= tBoxAward[1][2][3] then
				...
				local nLv = random(tBoxAward[1][2][1],tBoxAward[1][2][2]);
				...
				local nLv = random(tBoxAward[1][2][4],tBoxAward[1][2][5]);
			...
			if nRandThree <= tBoxAward[1][4][6] then
				...
				Msg2Player("Chóc mõng b¹n nhËn ®­îc "..tBoxAward[1][4][1]..tBoxAward[1][4][5].."!");
			end
			if nRandFour <= tBoxAward[1][5][2] then
