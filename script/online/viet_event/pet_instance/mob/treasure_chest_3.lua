...
function main()
	...
	for i=1, GetTeamSize() do
    		...
    		if GetTask(TSK_PET_TAIXU_INFO) ~= 0 and mod(GetTask(TSK_PET_TAIXU_DATE), 10) ~= 0 then
    			gf_AddItemEx2({2,1,30159, tPetFoodNum[nZone]}, "Tói thùc phÈm", "Thai Hu Huyen Canh Ban Dong Hanh", "nhËn th­ëng ru¬ng lo¹i 3")
    		end
    		local nPoint = floor(GetTask(TSK_PET_TAIXU_POINT) / 1000)
    		if nPoint < 2000000 then
    			SetTask(TSK_PET_TAIXU_POINT, GetTask(TSK_PET_TAIXU_POINT) + 3000)
    		end
    	end
