...
function main()
	...
	if MS_STATE_PEACE == nState or MS_STATE_READY == nState then
		...
		local szTitle="<color=green>Ti“n tuy’n qu©n TËng<color>".." cﬂn "..tostring(READY_TIME/18*nLoop).."s, chi’n tr≠Íng li™n Æ u ch›nh th¯c bæt Æ«u. SË ng≠Íi hi÷n tπi lµ: "..CampOne_Name.."["..nPlayerOne.."]:"..CampTwo_Name.."["..nPlayerTwo.."]"
		Say(szTitle, 0);
	else
		if MS_STATE_COMPLETE ~= nState and GetFightState() == 0 then
			...
			if nX == 1623 and nY == 3734 then
				tinsert(tSay, format("ß≠a ta Æ’n tr≠Ìc Æπi doanh/#KF_TransmitPlayer(%d,%d,%d)", 0, 1642, 3691));
			else
				tinsert(tSay, format("ß≠a ta Æ’n b™n g«n Æπi doanh/#KF_TransmitPlayer(%d,%d,%d)", 0, 1642, 3831));
			end
			local tRevivePos = {
