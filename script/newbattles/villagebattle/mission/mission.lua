...
function EndMission()
	...
	if GetMissionV(MV_WINNER) == 0 then	--Èç¹ûÊÇÊ±¼äµ½ÁË¶ø½áÊøµÄ
		...
		if nStepSong > nStepLiao then
			...
			Msg2SubWorld("Qua nh÷ng håi giao tranh ¸c liÖt, phe Tèng giµnh th¾ng lîi chung cuéc t¹i Nh¹n M«n Quan-chiÕn dŞch Gi¶i Cøu N«ng Trang!");
		elseif nStepSong < nStepLiao then
			SetMissionV(MV_WINNER,LIAO_ID);
			Msg2SubWorld("Qua nh÷ng håi giao tranh ¸c liÖt, phe Liªu giµnh th¾ng lîi chung cuéc t¹i Nh¹n M«n Quan-chiÕn dŞch Gi¶i Cøu N«ng Trang!");
		else
			SetMissionV(MV_WINNER,ALL_ID);
			Msg2SubWorld("Qua nh÷ng håi giao tranh ¸c liÖt, Tèng-Liªu hai bªn bÊt ph©n th¾ng b¹i t¹i Nh¹n M«n Quan-chiÕn dŞch Gi¶i Cøu N«ng Trang!");
		end;
	end;
