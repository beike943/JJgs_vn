...
function OnDeath(npcIndex)
	...
	if PlayerIndex == 0 then	--Èç¹ûÊÇ±»NPCÉ±ËÀ¶ø´¥·¢µÄ£¬PlayerIndexÔòÎª0
...
function process_player_kill_npc(npcIndex)
	...
	if nNpcType == TYPE_REFUGEE then	--Èç¹ûÉ±µÄÊÇÍÁÃñ£¬¾Í¼ÇÒ»ÏÂ
		if nKillVillagerPoint < KILL_VILLAGER_POINT_LIMITED then	
			BT_AddPersonalPoint(nPoint,0,1);	--¸øÓèÍæ¼ÒÒ»¶¨µÄ¸öÈË»ý·Ö
...
function process_camp_fight_camp(nPlayerCamp,sNpcName,nNpcIndex)
		...
		if GetMissionV(MV_KILL_SONG_LOOEY_COUNT+nEnemyCamp-1) == LOOEY_COUNT then
			if GetMissionV(MV_KILL_SONG_CAPTAIN_COUNT+nEnemyCamp-1) == CAPTAIN_COUNT then
				if nEnemyNpcType == 0 then
					Msg2MSAll(MISSION_ID,tCampNameZ[nEnemyCamp].."hiÖu óy vµ ®« thèng phe "..tCampNameZ[nEnemyCamp].." bÞ ®¸nh b¹i, tiªn phong phe ");
				end;
		...
		if GetMissionV(MV_KILL_SONG_CAPTAIN_COUNT+nEnemyCamp-1) == CAPTAIN_COUNT then
			...
			if GetMissionV(MV_KILL_SONG_PIONEER_COUNT+nEnemyCamp-1) == PIONEER_COUNT then
				if nEnemyNpcType == 1 then
					Msg2MSAll(MISSION_ID,tCampNameZ[nEnemyCamp].." xuÊt hiÖn. Tiªn phong "..tCampNameZ[nEnemyCamp].." bÞ ®¸nh b¹i, §¹i t­íng xuÊt hiÖn. ");
				end;
		...
		if GetMissionV(MV_KILL_SONG_PIONEER_COUNT+nEnemyCamp-1) == PIONEER_COUNT then
			if GetMissionV(MV_KILL_SONG_GENERAL_COUNT+nEnemyCamp-1) == GENERAL_COUNT then
				if nEnemyNpcType == 2 then
					Msg2MSAll(MISSION_ID,tCampNameZ[nEnemyCamp].." §¹i t­íng vµ Tiªn phong phe "..tCampNameZ[nEnemyCamp].." bÞ ®¸nh b¹i, Nguyªn So¸i xuÊt hiÖn. ");
				end;
	...
	elseif sNpcTypeName == "T­íng qu©n" then
		...
		if GetMissionV(MV_KILL_SONG_GENERAL_COUNT+nEnemyCamp-1) == GENERAL_COUNT then
			if GetMissionV(MV_KILL_SONG_PIONEER_COUNT+nEnemyCamp-1) == PIONEER_COUNT then
				if nEnemyNpcType == 2 then
					Msg2MSAll(MISSION_ID,tCampNameZ[nEnemyCamp].." §¹i t­íng tiªn phong bÞ ®¸nh b¹i,"..tCampNameZ[nEnemyCamp].." bÞ ®¸nh b¹i, Nguyªn So¸i xuÊt hiÖn. ");
				end;
	...
	elseif sNpcTypeName == "Nguyªn So¸i" then
		...
		if PlayerIndex > 0 then
			Msg2MSAll(MISSION_ID,GetName().." ®· ®¸nh b¹i, Nguyªn So¸i phe "..tCampNameZ[nEnemyCamp]..", "..tCampNameZ[nPlayerCamp].."_giµnh th¾ng lîi cuèi cïng.");
		else
			Msg2MSAll(MISSION_ID,GetNpcName(npcIndex).." ®· ®¸nh b¹i, Nguyªn So¸i phe "..tCampNameZ[nEnemyCamp]..", "..tCampNameZ[nPlayerCamp].."_giµnh th¾ng lîi cuèi cïng.");
		end;
...
function tell_end(tArg)
	SetFightState(0);
	Say(tBattleName[BATTLE_TYPE].." ®· kÕt thóc, b¹n giÕt ®­îc <color=yellow>"..BT_GetData(PTNC_KILL).."<color> ng­êi, tö trËn <color=yellow>"..BT_GetData(PTNC_DEATH).."<color> lÇn. C¸c vÞ ®· rÊt nç lùc, h·y chuÈn bÞ nhËn phÇn th­ëng cña Hoµng ®Õ!",0);
end;
--¸ù¾ÝNPCÃû×Ö»ñÈ¡NPCµÄÕóÓª
function get_npc_camp(szNpcName)
	local nNpcCamp = 0;
	if strsub(szNpcName,1,2) == "Tèng" then
		nNpcCamp = SONG_ID;
	elseif strsub(szNpcName,1,2) == "Liªu" then
		nNpcCamp = LIAO_ID;
	else
		nNpcCamp = ALL_ID;
	end;
